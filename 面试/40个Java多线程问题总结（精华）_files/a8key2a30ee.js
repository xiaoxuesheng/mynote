...
var s=d[n],p=s.getAttribute("data-backsrc")||s.getAttribute("data-src")||"";
if(t[p]){
...
var s=o.app_info.down_count||0,d=o.app_info.app_size||0,p=o.app_info.app_name||"",m=o.app_info.category,c=["万","百万","亿"];
if(s>=1e4){
s/=1e4;
for(var l=0;s>=10&&2>l;)s/=100,l++;
s=s.toFixed(1)+c[l]+"次";
}else s=s.toFixed(1)+"次";
d>=1024?(d/=1024,d=d>=1024?(d/1024).toFixed(2)+"MB":d.toFixed(2)+"KB"):d=d.toFixed(2)+"B",
m=m?m[0]||"其他":"其他";
for(var _=["-","(",":",'"',"'","：","（","—","“","‘"],f=-1,u=0,g=_.length;g>u;++u){
var w=_[u],v=p.indexOf(w);
-1!=v&&(-1==f||f>v)&&(f=v);
}
-1!=f&&(p=p.substring(0,f)),o.app_info._down_count=s,o.app_info._app_size=d,o.app_info._category=m,
...
for(var m=+new Date,d=[],c=this.sw,u=0,w=t.length;w>u;u++){
var p=t[u],f=p.el.offsetTop;
if(!p.show&&(i>=f&&i<=f+p.height+r||f>i&&i+o+n>f)){
var g=p.src,v=this;
...
!!v.onload&&v.onload(g,t);
},h(p.el,"src",g),d.push(g),p.show=!0,l(p.el,"visibility","visible","important");
}
a.isWp&&1*p.el.width>c&&(p.el.width=c);
}
...
var p=1*c.dataset.ratio,f=1*c.dataset.w||a;
"number"==typeof p&&p>0?(f=a>=f?f:a,w=f*p,c.style.width&&c.setAttribute("_width",c.style.width),
l(c,"width",f+"px","important"),l(c,"visibility","visible","important"),c.setAttribute("src",m)):l(c,"visibility","hidden","important");
...
var a=t("biz_wap/utils/mmversion.js"),r=t("biz_common/dom/event.js"),s=t("biz_common/dom/attr.js"),h=s.attr,l=s.setProperty,m=t("biz_common/ui/imgonepx.js");
return n;
