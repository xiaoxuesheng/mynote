...
{*/
    $(function()
    {
        ...
        function handle(obj)
        {
            ...
            {
                selectext(obj);
                /*for(var p=0;p<regarr.length;p++)
                {
                    var Regs = eval(regarr[p]);
                    Regs.flg = false;
                    callback(Regs,arrWord[p].url,arrWord[p].tip);
                }*/
                //恢复HTML标签
            ...
            function callback(Reg,arrUrl,tip)
            {
                if(Reg.flg)
                {
                    return false;
                }
                temp = temp.replace(Reg,function(word){
                    Reg.flg=true;
                    return "<a href='"+arrUrl+"' class='replace_word' title='"+tip+"' target='_blank' style='color:#df3434; font-weight:bold;'>"+word+"</a>";
                });
            }
            //参考知识库
            function relate(){
                ...
                if(arrTip.length<=0)
                {
                    ...
                    {
                        relateHtml += '<dl class="relate_list" ><dt><a href="'+arrUrl[i]+'"><img src="'+arrLogo[i]+'" alt="img"/></a></dt><dd><h4><a href="'+arrUrl[i]+'">'+arrTip[i]+'</a></h4><p><label><span>'+arrSubCount[i]+'</span><em>关注</em><i>|</i><span>'+arrConCount[i]+'</span><em>收录</em></label></p></dd></dl>'
                    }
